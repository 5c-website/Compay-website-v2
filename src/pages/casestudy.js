...
import image from '../Assets/privacypolicy/privacy-policy-bannr.jpg'
...
function Post({data,location}) {
  const strapiContent = data.solutions.nodes[0];
                    ...
                    <h1 className=' w-full  tracking-[0] mb-[16px] text-[56px] leading-[120%] text-[#fff]'>
                        ...
                        <div className=' w-[1200px] max-w-full flex flex-col justify-center items-center'>
                            <div className='blog_content text-[16px] leading-[150%]' style={{color:'rgba(255,255,255,0.8)'}}>
                                <p className='mb-[12px] texxt-[18px] leading-[170%]' style={{color:'rgba(255,255,255,0.8)'}}>
                                <div dangerouslySetInnerHTML={{ __html: content.case_study_content.data.case_study_content }}></div>
                                </p>
                                <div className=''>
...
query ($slug: String!){
  casestudyposts: allStrapiCasestudypost(filter: { slug: { eq: $slug } }) {
    nodes {
      Heroimage {
        localFile {
          url
