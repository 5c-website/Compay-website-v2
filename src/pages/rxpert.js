...
function Rxpert({ data }) {
  const Content = data.allStrapiRxpert.nodes[0];
  
  const handleBookDemoClick = () => {
    navigate('/get-in-touch', { state: { osteocheckData: ['Radiologist', 'Others'],Title:"Rxpert" } });
  };
        ...
        {point.bodycontent.data.bodycontent}
                  ...
                  <div className=' w-full max-w-full mt-7 flex sm:justify-center'>
                    <button role="button" aria-label="reachus"
                      class="btn btn-2 hover-slide-up uppercase" onClick={handleBookDemoClick}>
                      <span>Reach Us</span>
                    </button>
                  </div>
                </div>
